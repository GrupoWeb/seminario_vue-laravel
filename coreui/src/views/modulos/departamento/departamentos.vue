    ...
    export default {
     ...
     methods: {
         getDepartamenById(id) {
             axios.post(router[1].post.departamentById + '?token=' + localStorage.getItem("api_token"),
             {
                 id: id
             })
             .then(response => {
         ...
         getDepartamento(){
             axios.get(router[1].get.departamentos + '?token=' + localStorage.getItem("api_token"))
             .then(response => {
        ...
        close() {
         ...
         handleOk(id){
            axios.post(router[1].post.deleteDepartament + '?token=' + localStorage.getItem("api_token"), {
                id: id
            }).then(response => {
                if(response.data == 1) {
                    this.$swal({
        ...
        submit() {
            if(this.$refs.form_edit.checkValidity()){
                axios.post(router[1].post.updateDepartamenoById + '?token=' + localStorage.getItem("api_token"),{
                id: this.form.id,
